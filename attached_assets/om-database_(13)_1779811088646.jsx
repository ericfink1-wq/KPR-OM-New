...
- OPTIONS: put renewal options in renewalOptions; if an "Annual Option Rent" block gives the rent during options, pair each rate with its date, e.g. "1st 5Yr @ $26.40 (Nov-27)". Do not let option dates or option rents leak into rentSchedule or the current annualRent/rentPerSF.`;
      ...
      if (!tenants || !tenants.length) { setStatus("Couldn't read tenants from that PDF — try a clearer rent roll."); setBusy(false); return; }
      ...
      setStatus(`✓ Refreshed ${tenants.length} tenants${dateFound?` as of ${asOf}`:` (no date on the roll — stamped today, ${asOf})`}. Recomputed metrics where not locked. Verify before relying on them.`);
