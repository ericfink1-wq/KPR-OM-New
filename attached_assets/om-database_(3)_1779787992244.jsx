...
async function extractWithRecovery(text, onMsg) {
  // Initial extraction
  const first = await callExtractionAPI(EXTRACTION_PROMPT + "\n\nOM TEXT:\n" + text, onMsg);
  let extracted;
...
REQUIRED SCHEMA:
{
  ...
  "keyAssumptions": ["IMPORTANT — array of deal-level footnotes/assumptions from the rent roll or property summary that affect underwriting, e.g. 'Seller covering gap rent on Suite 120 until Mar-26', 'NOI assumes Tenant X exercises its option', 'Includes a $250K leasing credit at closing', 'Proforma rents used for 3 vacant suites'. These are HIGH IMPORTANCE — capture every such footnote. Empty array if none."],
...
- FOOTNOTES & ASSUMPTIONS: scour the rent roll and property/financial summary for footnotes and underwriting assumptions about tenants (gap rent covered by seller, assumed option exercise, abatements, proforma/estimated rents, unexecuted LOIs). Capture each in the tenant's assumptionNote and/or deal-level keyAssumptions. These materially affect value and are high priority.
...
export default function App() {
  ...
  async function processOne(item) {
    ...
    try {
      ...
      const extracted = await extractWithRecovery(text, onMsg);
      bump(95);
  ...
  async function reanalyzeOne(item) {
    ...
    try {
      ...
      const extracted = await extractWithRecovery(src.text, onMsg);
      bump(95);
      let updated = null;
      setDeals(prev => prev.map(d => {
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      className="app-shell"
      style={{ fontFamily:"'Inter',-apple-system,sans-serif", minHeight:"100vh", color:"#383a37", display:"flex", flexDirection:"column", WebkitFontSmoothing:"antialiased" }}>
                  ...
                  <>
                    ...
                    {portfolio.count===0 ? (
                      ...
                      <div style={{ display:"flex", gap:11, flexWrap:"wrap" }}>
                        ...
                        <div style={{ flex:"2 1 240px", background:"#ffffff", border:"1px solid #efe8da", borderRadius:12, padding:"13px 16px", boxShadow:"0 1px 2px rgba(56,58,55,0.04)" }}>
                          <div style={{ fontSize:10, letterSpacing:"0.06em", color:"#a69e91", marginBottom:7, fontWeight:500, textTransform:"uppercase" }}>Largest Tenants by Base Rent</div>
                          {portfolio.topByRent.length ? (
                            <div style={{ display:"flex", flexDirection:"column", gap:3 }}>
                              {portfolio.topByRent.map((t,i)=>(
                                <div key={i} style={{ display:"flex", justifyContent:"space-between", gap:10, fontSize:12 }}>
                                  <span style={{ color:"#383a37", fontWeight:500, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{t.name}</span>
                                  <span style={{ color:"#0f9d63", fontWeight:600, flexShrink:0 }}>{fmtM(Number(t.annualRent))}</span>
                                </div>
                              ))}
                            </div>
                          ) : <div style={{ fontSize:13, color:"#c2b6a5" }}>—</div>}
...
function DetailView({ deal:d, allDeals, editor, onEditor, onToggleVerified, onLookupSale, saleBusy, onGetDemo, demoBusy, onBack, onDelete, onUpdate, onCompare, onQuery }) {
  const [confirmDel, setConfirmDel] = useState(false);
  const [editNotes, setEditNotes] = useState(false);
  const [notesVal, setNotesVal] = useState(d.userNotes||"");
  // Transaction / deal-stage fields (user-entered), synced locally and saved on blur
  const [txn, setTxn] = useState({
    txnPurchasePrice: d.txnPurchasePrice ?? "", txnSeller: d.txnSeller ?? "",
    txnLoiDate: d.txnLoiDate ?? "", txnCloseDate: d.txnCloseDate ?? "",
    txnSalePrice: d.txnSalePrice ?? "", txnBuyer: d.txnBuyer ?? "",
    txnSaleDate: d.txnSaleDate ?? "", txnBroker: d.txnBroker ?? "",
    // Disposition record (filled once sold)
    dispExitCap: d.dispExitCap ?? "", dispCosts: d.dispCosts ?? "", dispLoanPayoff: d.dispLoanPayoff ?? "",
    dispNotes: d.dispNotes ?? "",
    // Acquisition record (filled once owned)
    acqCapRate: d.acqCapRate ?? "", acqNOIAtClose: d.acqNOIAtClose ?? "", acqEntity: d.acqEntity ?? "",
    acqBroker: d.acqBroker ?? "", acqContractDate: d.acqContractDate ?? "", acqDDExpiration: d.acqDDExpiration ?? "",
    acqDeposit: d.acqDeposit ?? "", acqClosingCosts: d.acqClosingCosts ?? "", acqFee: d.acqFee ?? "",
    acqTitleCo: d.acqTitleCo ?? "", acqCounsel: d.acqCounsel ?? "", acqPropManager: d.acqPropManager ?? "",
    acqStrategy: d.acqStrategy ?? "", acqHoldPeriod: d.acqHoldPeriod ?? "", acqTargetIRR: d.acqTargetIRR ?? "",
    acqNotes: d.acqNotes ?? "",
    // Financing / debt
    debtLender: d.debtLender ?? "", debtType: d.debtType ?? "", debtLoanAmount: d.debtLoanAmount ?? "",
    debtRate: d.debtRate ?? "", debtRateType: d.debtRateType ?? "", debtIndex: d.debtIndex ?? "",
    debtSpread: d.debtSpread ?? "", debtOriginationDate: d.debtOriginationDate ?? "", debtMaturityDate: d.debtMaturityDate ?? "",
    debtTermYears: d.debtTermYears ?? "", debtAmortYears: d.debtAmortYears ?? "", debtIOPeriod: d.debtIOPeriod ?? "",
    debtLTV: d.debtLTV ?? "", debtDSCR: d.debtDSCR ?? "", debtRecourse: d.debtRecourse ?? "",
    debtPrepay: d.debtPrepay ?? "", debtExtensions: d.debtExtensions ?? "", debtEscrows: d.debtEscrows ?? "",
    debtAssumable: d.debtAssumable ?? "", debtLoanNumber: d.debtLoanNumber ?? "", debtContact: d.debtContact ?? "",
    debtNotes: d.debtNotes ?? "",
  });
  const saveTxn = (key) => {
    const raw = txn[key];
    const numKeys = ["txnPurchasePrice","txnSalePrice","debtLoanAmount","debtRate","debtSpread","debtTermYears","debtAmortYears","debtIOPeriod","debtLTV","debtDSCR","acqCapRate","acqNOIAtClose","acqDeposit","acqClosingCosts","acqFee","acqHoldPeriod","acqTargetIRR","dispExitCap","dispCosts","dispLoanPayoff"];
    const val = numKeys.includes(key) ? (raw==="" ? null : Number(String(raw).replace(/[^0-9.]/g,""))) : (raw==="" ? null : raw);
    onUpdate(d.id, { [key]: val });
  };
  const r = getRecency(d);
  ...
  return (
    <div style={{ flex:1, overflowY:"auto", padding:"20px 24px" }}>
      ...
      {(() => {
        ...
        const TF = ({ label, field, placeholder, prefix, suffix, options, wide }) => (
          <div style={{ display:"flex", flexDirection:"column", gap:5, gridColumn: wide?"1 / -1":"auto" }}>
            ...
            {options ? (
              <select value={txn[field]} onChange={e=>{ setTxn(t=>({ ...t, [field]: e.target.value })); onUpdate(d.id, { [field]: e.target.value || null }); }}
              ...
              <div style={{ display:"flex", alignItems:"center", background:"#f5f1e8", border:"1px solid #e6dfd0", borderRadius:8, padding:"0 10px" }}>
                ...
                <input
                  value={txn[field]}
                  onChange={e=>setTxn(t=>({ ...t, [field]: e.target.value }))}
                  onBlur={()=>saveTxn(field)}
        ...
        return (
          <div style={{ background:"#ffffff", border:"1px solid #efe8da", borderRadius:12, padding:"18px 20px", marginBottom:14, boxShadow:"0 1px 2px rgba(56,58,55,0.04)" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(200px, 1fr))", gap:14, marginBottom:18 }}>
              ...
              <TF label="LOI Date" field="txnLoiDate" placeholder="YYYY-MM-DD"/>
              ...
              {owned && <>
                ...
                <Grp title="Key Dates"/>
                <TF label="PSA / Contract Date" field="acqContractDate" placeholder="YYYY-MM-DD"/>
                <TF label="Due-Diligence Expiration" field="acqDDExpiration" placeholder="YYYY-MM-DD"/>
                ...
                <TF label="Title Company" field="acqTitleCo" placeholder="title / escrow"/>
                <TF label="Legal Counsel" field="acqCounsel" placeholder="acquisition counsel"/>
                <TF label="Property Manager" field="acqPropManager" placeholder="PM company"/>
                ...
                <TF label="Business Plan / Notes" field="acqNotes" placeholder="thesis, value-add plan, key risks" wide/>
              </>}
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(200px, 1fr))", gap:14 }}>
              ...
              {sold && <>
                ...
                <TF label="Disposition Notes" field="dispNotes" placeholder="rationale, terms, buyer profile" wide/>
              </>}
            </div>
      ...
      {(() => {
        const owned = d.status === "Owned" || d.status === "Sold";
        const F = ({ label, field, placeholder, prefix, suffix, options, wide }) => (
          <div style={{ display:"flex", flexDirection:"column", gap:5, gridColumn: wide?"1 / -1":"auto" }}>
            <label style={{ fontSize:11, color:"#a69e91", fontWeight:600, letterSpacing:"0.03em", textTransform:"uppercase" }}>{label}</label>
            {options ? (
              <select value={txn[field]} onChange={e=>{ setTxn(t=>({ ...t, [field]: e.target.value })); onUpdate(d.id, { [field]: e.target.value || null }); }}
                style={{ background:"#f5f1e8", border:"1px solid #e6dfd0", borderRadius:8, padding:"10px 10px", fontSize:14, color:"#383a37", outline:"none" }}>
                <option value="">—</option>
                {options.map(o=><option key={o} value={o}>{o}</option>)}
              </select>
            ) : (
              <div style={{ display:"flex", alignItems:"center", background:"#f5f1e8", border:"1px solid #e6dfd0", borderRadius:8, padding:"0 10px" }}>
                {prefix && <span style={{ color:"#a69e91", fontSize:14 }}>{prefix}</span>}
                <input value={txn[field]} onChange={e=>setTxn(t=>({ ...t, [field]: e.target.value }))} onBlur={()=>saveTxn(field)} placeholder={placeholder}
                  style={{ flex:1, background:"transparent", border:"none", outline:"none", padding:"10px 6px", fontSize:14, color:"#383a37" }}/>
                {suffix && <span style={{ color:"#a69e91", fontSize:13 }}>{suffix}</span>}
              </div>
            )}
        ...
        return (
          <div style={{ background:"#ffffff", border:"1px solid #efe8da", borderRadius:12, padding:"18px 20px", marginBottom:14, boxShadow:"0 1px 2px rgba(56,58,55,0.04)" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(200px, 1fr))", gap:14 }}>
              ...
              <F label="Loan Number" field="debtLoanNumber" placeholder="Servicer loan #"/>
              <F label="Lender Contact" field="debtContact" placeholder="Name / email / phone" wide/>
              ...
              <F label="DSCR at Close" field="debtDSCR" placeholder="e.g. 1.35" suffix="x"/>
              ...
              <F label="Notes" field="debtNotes" placeholder="Anything else worth recording" wide/>
            </div>
