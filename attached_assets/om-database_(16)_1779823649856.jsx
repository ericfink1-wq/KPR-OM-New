...
// ── Tenant name canonicalization ─────────────────────────────────────────────
// OMs spell the same brand many ways ("Ulta", "ULTA", "Ulta Salon",
// "Ulta Beauty"). To group a tenant correctly across deals we reduce every name
// to one stable canonical KEY, in two layers:
//   1) mechanical normalization (case, punctuation, store #s, corporate suffixes,
//      and a short list of redundant trailing format words like "Salon"/"Beauty"),
//   2) an editable alias map for brand variants normalization can't safely catch.
// If a brand still splits in the database, add ONE line to TENANT_ALIASES below:
//      "normalized variant": "Canonical Display Name",
// The left side must be the *normalized* form (lowercase, no punctuation, e.g.
// "at and t" for AT&T). Truly different businesses that merely share a generic
// word (e.g. "Pet Supermarket" vs "Pet Supplies Plus") are kept separate.
const TENANT_ALIASES = {
  // left = normalized variant      →  right = clean canonical display name
  "ulta": "Ulta Beauty",
...
// Redundant format/descriptor words that are safe to drop from the END of a name.
...
function _normTenant(name) {
  let s = (name || "").toString().normalize("NFD").replace(/[\u0300-\u036f]/g, ""); // strip accents
  s = s.toLowerCase()
       .replace(/['.]/g, "")          // drop apostrophes & periods (Macy's->macys, T.J.->tj)
       .replace(/&/g, " and ")         // & -> and
       .replace(/#\s*\d[\d-]*/g, " ")  // drop store numbers (#1234, # 0040-1)
       .replace(/[^a-z0-9]+/g, " ")    // any other punctuation -> space
       .replace(/\s+/g, " ")
       .trim();
  if (s.startsWith("the ")) s = s.slice(4);
  let parts = s.split(" ").filter(Boolean);
  // peel redundant trailing words / trailing store numbers while a base remains
...
// Clean, human-readable canonical name for labels/headers.
function tenantLabel(name) {
  const n = _normTenant(name);
  if (TENANT_ALIASES[n]) return TENANT_ALIASES[n];
  return n.replace(/\b\w/g, c => c.toUpperCase()) || (name || "");
}
...
export default function App() {
  ...
  async function doCombine(primaryId, ids) {
    ...
    setDeals(prev => {
      ...
      for (const a of concatArr("anchors")) { const k = tenantKey(typeof a === "string" ? a : a?.name || ""); if (k && !seen.has(k)) { seen.add(k); anchors.push(a); } }
  ...
  const stats = {
    ...
    tenantBrands: [...new Set(activeDeals.flatMap(d=>(d.tenants||[]).map(t=>tenantKey(t.name))).filter(Boolean))].length,
...
function TenantView({ tenantName, deals, onBack, onOpenDeal }) {
  ...
  const num = v => (v==null||v===""||isNaN(Number(v))) ? null : Number(v);
  const rows = [];
  (deals||[]).forEach(d => (d.tenants||[]).forEach(t => { if (tenantKey(t.name) === target) rows.push({ deal:d, t }); }));
...
function CompareView({ deals:[a,b], onBack, onQuery }) {
  ...
  const oldT = new Set((older.tenants||[]).map(t=>tenantKey(t.name)));
  const newT = new Set((newer.tenants||[]).map(t=>tenantKey(t.name)));
