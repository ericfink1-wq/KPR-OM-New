...
const PROSPECT_STALE_DAYS = 61; // ~2 months

// ── Upload password gate ──────────────────────────────────────────────────────
// A lightweight password on the actions that spend API tokens (uploading OMs,
// refreshing a rent roll, the analyst chat, the per-property chat, and web
// sale/demographics lookups), so teammates can't run up a large bill.
...
function ensureUploadAllowed() {
  ...
  const entry = window.prompt("Enter the password to use AI features — uploads, analyst chat, and web lookups (this protects your API tokens):");
...
async function callExtractionAPI(content, onStatus) {
  const requestBody = JSON.stringify({
    // OM extraction runs on Haiku 4.5 — ~3x cheaper than Sonnet ($1/$5 vs $3/$15 per
    // 1M tokens) and the highest-volume call in the app. 64K max output covers the
    // 32K limit here. Chat/analyst/lookups stay on Sonnet for quality.
...
export default function App() {
  ...
  const [combineModal, setCombineModal] = useState(null); // { ids, primaryId } for combining phases into one owned deal
  ...
  async function handleFiles(e) {
    if (!ensureUploadAllowed()) { if (e.target) e.target.value = ""; return; }
  ...
  // Combine two+ OMs (e.g. phases of one property you bought) into ONE deal:
  // sizes and income are ADDED, tenant rosters are concatenated, per-area metrics
  // (occupancy, WALT, cap, rent/SF) are recomputed. The extra OMs go to Trash. Undo
  // restores the primary's pre-combine state AND un-trashes the extras.
  async function doCombine(primaryId, ids) {
    const others = ids.filter(id => id !== primaryId);
    if (!others.length) return;
    const ts = new Date().toISOString();
    const primarySnapshot = deals.find(d => d.id === primaryId);

    // Images: keep the primary's cover; pull one from an extra if the primary lacks it; concat site plans.
    try {
      const pImgs = (await idbLoadImages(primaryId).catch(()=>null)) || {};
      let cover = pImgs.cover || null, coverThumb = pImgs.coverThumb || null;
      let sitePlan = Array.isArray(pImgs.sitePlan) ? [...pImgs.sitePlan] : [];
      for (const oid of others) {
        const oi = await idbLoadImages(oid).catch(()=>null);
        if (!oi) continue;
        if (!cover && oi.cover) { cover = oi.cover; coverThumb = oi.coverThumb || null; }
        if (Array.isArray(oi.sitePlan)) sitePlan = sitePlan.concat(oi.sitePlan);
      }
      await idbSaveImages(primaryId, { cover, coverThumb, sitePlan, pagePicks: pImgs.pagePicks || [], needsSitePlanPick: false });
    } catch {}

    setDeals(prev => {
      const P = prev.find(d => d.id === primaryId); if (!P) return prev;
      const os = prev.filter(d => others.includes(d.id));
      const all = [P, ...os];
      const num = v => (v == null || v === "" || isNaN(Number(v))) ? null : Number(v);
      const combined = { ...P };

      // 1. Additive size / income fields — sum across all OMs.
      ...
      for (const f of SUM) { let s = 0, any = false; for (const d of all) { const v = num(d[f]); if (v != null) { s += v; any = true; } } if (any) combined[f] = s; }

      // 2. Concatenate tenant rosters and list fields.
      const concatArr = f => all.flatMap(d => Array.isArray(d[f]) ? d[f] : []);
      ...
      for (const a of concatArr("anchors")) { const k = (typeof a === "string" ? a : a?.name || "").toLowerCase(); if (k && !seen.has(k)) { seen.add(k); anchors.push(a); } }
      if (anchors.length) combined.anchors = anchors;

      // 3. Recompute per-area / derived metrics from the combined totals.
      ...
      for (const d of all) { const sf = num(d.totalSF), oc = num(d.occupancy), w = num(d.walt); if (sf && oc != null) { occW += oc * sf; occSF += sf; } if (sf && w != null) { waltW += w * sf; waltSF += sf; } }
      if (occSF) combined.occupancy = +(occW / occSF).toFixed(1);
      if (waltSF) combined.walt = +(waltW / waltSF).toFixed(1);
      const baseRent = combined.tenants.reduce((s, t) => s + (num(t.annualRent) || 0), 0) || totGPR;
      if (baseRent && totSF) combined.weightedAvgRentPSF = +(baseRent / totSF).toFixed(2);

      // 4. Combine narrative text (dedupe identical blocks).
      const join = f => { const parts = all.map(d => d[f]).filter(Boolean); return parts.length ? Array.from(new Set(parts)).join("\n\n") : P[f]; };
      if (all.some(d => d.notes)) combined.notes = join("notes");
      if (all.some(d => d.userNotes)) combined.userNotes = join("userNotes");

      // 5. Preserve verified flags from every OM.
      const ver = { ...(P.verified || {}) };
      for (const o of os) for (const vf of Object.keys(o.verified || {})) if (!ver[vf]) ver[vf] = o.verified[vf];
      combined.verified = ver;

      const otherNames = os.map(o => o.propertyName || o.fileName || "deal").join(", ");
      combined.editHistory = [...(P.editHistory || []), { ts, by: editor || "Unknown", changes: [{ field: "record", from: `${all.length} OMs`, to: `combined into one — summed SF/NOI/income and merged tenant rosters from: ${otherNames}` }] }];

      return prev.map(d => {
        if (d.id === primaryId) return combined;
        if (others.includes(d.id)) return { ...d, deleted: true, deletedAt: ts, editHistory: [...(d.editHistory || []), { ts, by: editor || "Unknown", changes: [{ field: "record", from: "active", to: `combined into ${P.propertyName || P.fileName || "primary"} (trashed)` }] }] };
        return d;
      });
    });

    setUndoBuf({ ids: others, label: `Combined ${others.length + 1} OMs into one`, restore: primarySnapshot ? [primarySnapshot] : null });
    setCombineModal(null); clearSelection();
    setNotice(`Combined ${ids.length} OMs into one deal — sizes, income, and tenants were added together. The extra OMs are in Trash if you need them back.`);
  }
  ...
  function undoDelete() {
    if (!undoBuf) return;
    if (undoBuf.restore && undoBuf.restore.length) {
      const byId = new Map(undoBuf.restore.map(d => [d.id, d]));
      setDeals(prev => prev.map(d => byId.has(d.id) ? byId.get(d.id) : d));
      if (selectedDeal && byId.has(selectedDeal.id)) setSelectedDeal(byId.get(selectedDeal.id));
    }
    restoreDeals(undoBuf.ids);
    setUndoBuf(null);
  }
  ...
  const filteredDeals = activeDeals.filter(d => {
    ...
    const statusRank = { "Owned":0,"Under Contract":1,"Prospect":2,"Sold":3,"Passed":4 };
    let av, bv;
    switch (sortBy) {
    ...
    if (av>bv) return 1*dir;
    if (sortBy==="status") { // within a status group, newest first
      const ra=new Date(a.omDate||a.uploadedAt||0).getTime(), rb=new Date(b.omDate||b.uploadedAt||0).getTime();
      return rb-ra;
    }
    return 0;
  });
  ...
  return (
    <div
      ...
      {combineModal && (() => {
        ...
        const sum = f => sel.reduce((s, d) => s + num(d[f]), 0);
        ...
        return (
          <div style={{ position:"fixed", inset:0, background:"rgba(10,14,23,0.92)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:95, padding:16 }}>
            <div style={{ background:"#faf7f0", border:"1px solid #383a37", borderRadius:12, padding:24, maxWidth:540, width:"100%", maxHeight:"85vh", overflowY:"auto" }}>
              <div style={{ fontFamily:"'Fraunces',serif", fontWeight:700, fontSize:15, color:"#26281f", marginBottom:4 }}>Combine {sel.length} OMs into one deal</div>
              <p style={{ fontSize:11.5, color:"#3f7a1f", lineHeight:1.55, margin:"0 0 14px" }}>
                For <strong>phases or parcels of one property you own</strong>. Square footage, NOI, income, and tenant rosters are <strong>added together</strong>; occupancy, WALT, cap rate, and rent/SF are recalculated from the totals. The extra OMs move to Trash (you can undo).
              </p>
              <div style={{ display:"flex", gap:10, marginBottom:14, flexWrap:"wrap" }}>
                {[["Combined SF", totSF>0?totSF.toLocaleString():"—"],["Combined NOI", fmt$(totNOI)],["Total tenants", String(totTenants)]].map(([l,v])=>(
                  <div key={l} style={{ flex:"1 1 120px", background:"#fff", border:"1px solid #e7e0d2", borderRadius:8, padding:"10px 12px" }}>
                    <div style={{ fontSize:9, letterSpacing:"0.06em", color:"#a69e91", textTransform:"uppercase", marginBottom:4 }}>{l}</div>
                    <div style={{ fontFamily:"'Fraunces',serif", fontSize:18, fontWeight:600, color:"#383a37" }}>{v}</div>
                  </div>
                ))}
              </div>
              <div style={{ fontSize:11, color:"#7d766a", marginBottom:8 }}>Keep this OM's name, address &amp; status for the combined deal:</div>
              {sel.map(d => (
                <label key={d.id} style={{ display:"flex", alignItems:"center", gap:10, background:"#fff", border:"1px solid "+(combineModal.primaryId===d.id?"#6dba43":"#e7e0d2"), borderRadius:8, padding:"10px 12px", marginBottom:8, cursor:"pointer" }}>
                  <input type="radio" name="combinePrimary" checked={combineModal.primaryId===d.id} onChange={()=>setCombineModal(m=>({...m, primaryId:d.id}))}/>
                  <div>
                    <div style={{ fontSize:12.5, color:"#2a2c27", fontWeight:600 }}>{d.propertyName||d.fileName}{combineModal.primaryId===d.id && <span style={{ color:"#0f9d63", fontSize:10, marginLeft:6 }}>PRIMARY</span>}</div>
                    <div style={{ fontSize:10.5, color:"#a69e91" }}>{d.address||""}{d.address?" · ":""}{d.totalSF?Number(d.totalSF).toLocaleString()+" SF":""}{d.tenants?.length?` · ${d.tenants.length} tenants`:""}</div>
                  </div>
                </label>
              ))}
              <div style={{ display:"flex", gap:8, marginTop:8, justifyContent:"flex-end" }}>
                <button onClick={()=>setCombineModal(null)} style={{ background:"transparent", border:"1px solid #e7e0d2", color:"#7d766a", padding:"7px 14px", borderRadius:8, cursor:"pointer", fontSize:12, fontFamily:"'Inter',sans-serif" }}>Cancel</button>
                <button onClick={()=>doCombine(combineModal.primaryId, combineModal.ids)} style={{ background:"#6dba43", border:"none", color:"#1f2b16", padding:"7px 14px", borderRadius:8, cursor:"pointer", fontSize:12, fontWeight:700, fontFamily:"'Inter',sans-serif" }}>Combine into one deal</button>
              </div>
            </div>
          </div>
        );
              ...
              <>
                ...
                {selectedIds.size > 0 && (
                  <div style={{ display:"flex", alignItems:"center", gap:12, flexWrap:"wrap", background:"#383a37", borderRadius:12, padding:"12px 18px", marginBottom:12, boxShadow:"0 4px 16px rgba(56,58,55,0.18)" }}>
                    ...
                    {/* Combine phases/parcels of one owned property — ADDS sizes, income, and tenants */}
                    <button onClick={openCombine} disabled={selectedIds.size<2}
                      title="Combine phases/parcels of ONE property you own into a single deal — SF, NOI, income, and tenant rosters are ADDED together. The extra OMs move to Trash."
                      style={{ background:"transparent", border:"1px solid "+(selectedIds.size>=2?"#6dba43":"#3f5a2e"), color:selectedIds.size>=2?"#6dba43":"#4e6a3e", padding:"7px 14px", borderRadius:8, cursor:selectedIds.size>=2?"pointer":"default", fontSize:12, fontWeight:600 }}>
                      Combine phases…
                    </button>
                ...
                {filteredDeals.map(d => {
                  ...
                  return (
                    <div key={d.id} onClick={()=>{ setSelectedDeal(d); setTab("detail"); }}
                      ...
                      <span style={{ color:STATUS_COLORS[d.status]||"#7d766a", fontSize:9 }}>{d.status||"—"}{d.autoPassed && <span title="Auto-passed after 2+ months as a prospect" style={{ color:"#b08968", fontSize:8, marginLeft:3, letterSpacing:"0.04em" }}>· AUTO</span>}</span>
                    </div>
                  );
                })}
              </>
...
// Refresh a property's tenant roster from a current rent roll PDF — useful when the OM
// is stale (e.g. an asset you've owned for years). Replaces the tenant list, stamps the
// "as of" date (the rent roll's own date if stated, else today), and recomputes
// tenant-derived metrics (occupancy, WALT, rent/SF) unless they're verified-locked.
function RentRollImport({ deal, onUpdate }) {
  ...
  // Shared apply step: turn a parsed { rentRollDate, tenants:[...] } object into a
  // roster update with recomputed metrics. Used by BOTH the PDF flow (extracts via
  // the API) and the paste flow (extracted in Claude chat — no API spend).
  function applyParsed(out) {
    const tenants = out && Array.isArray(out.tenants) ? out.tenants.filter(t=>t && t.name) : null;
    if (!tenants || !tenants.length) { setStatus("No tenants found in that data \u2014 check it and try again."); return false; }
    const today = new Date().toISOString().slice(0,10);
    const dateFound = out.rentRollDate && /\d{4}/.test(String(out.rentRollDate));
    const asOf = dateFound ? String(out.rentRollDate).slice(0,10) : today;
    const patch = { tenants, tenantsAsOf: asOf, tenantsSource: "Rent roll", rentRollDateFound: !!dateFound };
    // Recompute tenant-derived metrics from the fresh roster, but never overwrite a verified-locked field.
    const ver = deal.verified || {};
    const num = v => (v==null||v===""||isNaN(Number(v))) ? null : Number(v);
    const totSF = num(deal.totalSF);
    const leasedSF = tenants.reduce((s,t)=>s+(num(t.sf)||0),0);
    const totRent = tenants.reduce((s,t)=>s+(num(t.annualRent)||0),0);
    if (!ver.occupancy && totSF && leasedSF) patch.occupancy = +Math.min(100, leasedSF/totSF*100).toFixed(1);
    if (!ver.weightedAvgRentPSF && leasedSF && totRent) patch.weightedAvgRentPSF = +(totRent/leasedSF).toFixed(2);
    let wW=0, wSF=0; for (const t of tenants){ const sf=num(t.sf), r=num(t.remainingTermYears); if (sf && r!=null){ wW+=r*sf; wSF+=sf; } }
    if (!ver.walt && wSF) patch.walt = +(wW/wSF).toFixed(1);
    onUpdate(deal.id, patch);
    setStatus(`\u2713 Loaded ${tenants.length} tenants${dateFound?` as of ${asOf}`:` (no date given \u2014 stamped today, ${asOf})`}. Recomputed metrics where not locked. Verify before relying on them.`);
    return true;
  }
  function applyPaste() {
    let out = null; try { out = robustParseJSON(pasteVal); } catch {}
    if (!out) { setStatus("That doesn't look like valid JSON \u2014 paste the whole object (starts with { ends with })."); return; }
    if (applyParsed(out)) { setPasteVal(""); setShowPaste(false); }
  }
  ...
  async function handle(e) {
    ...
    if (!file) return;
    if (!ensureUploadAllowed()) return;
    setBusy(true); setStatus("Reading rent roll…");
    try {
      const { text } = await extractPdfText(file, null, false);
      setStatus("Extracting current tenants with AI…");
      const sys = `You extract a commercial real estate RENT ROLL from a PDF. Output ONLY a single JSON object: {"rentRollDate": "the 'as of' date printed on the rent roll as YYYY-MM-DD, or null if none is shown", "tenants": [ ... ]}. Each tenant uses exactly these keys (null when unknown): name (string), suite (string|null), sf (number), rentPerSF (number), annualRent (number), leaseStart (string), leaseExpiry (string), leaseType ("NNN"|"Gross"|"Modified Gross"|null), reimbursementMethod (string, e.g. "Pro-Rata NNN", "Fixed CAM/INS", "Gross"), renewalOptions (string|null), creditRating ("Investment Grade"|"Non-Investment Grade"|null), salesPSF (number|null), isAnchor (boolean), remainingTermYears (number|null — years from the rent roll date to lease expiry), rentBumps (string|null — a SHORT escalation pattern if one is obvious, e.g. "~3%/yr", else null), rentSchedule (string|null — the future in-term step schedule as date+rate pairs). For money use plain numbers (no $/commas); percentages as numbers.

READING A MULTI-COLUMN PROPERTY-MANAGEMENT RENT ROLL (e.g. Yardi/MRI exports with many side-by-side columns):
- annualRent = the CURRENT "Annual Rent" column (base rent only). Do NOT use "Gross Annual Rent"/"Gross Monthly Rent" (those add CAM, taxes & insurance), and do NOT use "Annual Rent Increase" or "Annual Option Rent" amounts (those are FUTURE scheduled bumps / renewal-option rents, never the current rent).
- rentPerSF = the "Rent/SF" column (current base rent PSF) — not an increase or option PSF.
- leaseStart = "Term Comm. Date"; leaseExpiry = "Term Exp. Date".
- name = the printed tenant name only. Strip any duplicate echo in parentheses, and IGNORE personal guarantor/owner names or DBA sub-lines printed beneath a tenant (e.g. "AT&T (AT&T) Rick Beasley, Keys Kennard and Todd Willis" -> name is "AT&T").
- leaseType/reimbursementMethod: if CAM, Taxes & Insurance are billed separately (non-zero) -> "NNN"; if those are $0 but base rent is positive -> "Gross" or "Modified Gross".
- isAnchor: true for large-format anchors (grocery, cinema, fitness, or big-box >= ~20,000 SF).
- SKIP these rows entirely: any "VACANT" row, any Total/Subtotal/Grand Total row, and any artifact row with 0 SF AND $0 rent (often a duplicate sub-line of a real tenant). KEEP real gross-lease tenants that show $0 CAM/Tax/Ins but a positive base rent.
- RENT STEPS (critical): the "Annual Rent Increase" block lists each future in-term step as an effective Date, then its new Annual amount, then its Incr/SF — all on the SAME row. Read ACROSS each row so every date keeps its own amount; never output step dates without amounts. Put them in rentSchedule as a concise list of rate+date pairs using the Incr/SF (PSF) value, e.g. "$19.06 Jan-28; $19.63 Jan-29; $20.22 Jan-30". If a simple pattern is obvious also set rentBumps (e.g. "~3%/yr"); otherwise leave rentBumps null.
- OPTIONS: put renewal options in renewalOptions; if an "Annual Option Rent" block gives the rent during options, pair each rate with its date, e.g. "1st 5Yr @ $26.40 (Nov-27)". Do not let option dates or option rents leak into rentSchedule or the current annualRent/rentPerSF.`;
      const user = `Rent roll text:\n${(text||"").slice(0,80000)}\n\nReturn ONLY the JSON object.`;
      const resp = await fetch("https://api.anthropic.com/v1/messages", {
        method:"POST", headers:{ "Content-Type":"application/json" },
        body: JSON.stringify({ model:"claude-haiku-4-5-20251001", max_tokens:32000, system: sys, messages:[{ role:"user", content:user }] })
      });
      const data = await resp.json();
    ...
    <div style={{ marginBottom:12, background:"#f3f7ee", border:"1px dashed #b8d49a", borderRadius:10, padding:"10px 14px" }}>
      <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
        ...
        <button onClick={()=>fileRef.current?.click()} disabled={busy}
          style={{ background: busy?"#e7ecde":"#fff", border:"1px solid #8cbf63", color:"#3f7a1f", padding:"8px 14px", borderRadius:8, cursor:busy?"default":"pointer", fontSize:12, fontWeight:600, fontFamily:"'Inter',sans-serif" }}>
          {busy ? "Refreshing…" : "⬆ Refresh tenants from a current rent roll (PDF)"}
        </button>
        <span style={{ fontSize:12, color: status.startsWith("\u2713")?"#3f7a1f":"#6f6a5f" }}>{status || "Replaces the roster with the current rent roll and stamps its date."}</span>
        <button onClick={()=>setShowPaste(v=>!v)} disabled={busy}
          style={{ background:"transparent", border:"1px solid #8cbf63", color:"#3f7a1f", padding:"8px 12px", borderRadius:8, cursor:busy?"default":"pointer", fontSize:12, fontWeight:600, fontFamily:"'Inter',sans-serif" }}>
          {showPaste ? "Close paste box" : "\u2398 Paste roster from Claude (no API)"}
        </button>
      </div>
      {showPaste && (
        <div style={{ marginTop:10 }}>
          <div style={{ fontSize:11, color:"#6f6a5f", lineHeight:1.55, marginBottom:6 }}>
            Extract the rent roll in Claude chat (no API spend), then paste the JSON object it returns here. It should look like <code style={{ background:"#eef3e6", padding:"1px 5px", borderRadius:4 }}>{`{ "rentRollDate": "YYYY-MM-DD", "tenants": [ \u2026 ] }`}</code>. This replaces the current roster and stamps the as-of date \u2014 same as the PDF refresh, just free.
          </div>
          <textarea value={pasteVal} onChange={e=>setPasteVal(e.target.value)} placeholder={`{ "rentRollDate": "2025-03-31", "tenants": [ \u2026 ] }`}
            style={{ width:"100%", minHeight:120, fontSize:11, fontFamily:"ui-monospace, Menlo, monospace", padding:"8px 10px", border:"1px solid #b8d49a", borderRadius:8, color:"#383a37", resize:"vertical", boxSizing:"border-box" }}/>
          <div style={{ display:"flex", gap:8, marginTop:8 }}>
            <button onClick={applyPaste} disabled={busy || !pasteVal.trim()}
              style={{ background:(busy||!pasteVal.trim())?"#e7ecde":"#6dba43", border:"none", color:"#26281f", padding:"8px 14px", borderRadius:8, cursor:(busy||!pasteVal.trim())?"default":"pointer", fontSize:12, fontWeight:700, fontFamily:"'Inter',sans-serif" }}>
              Apply pasted roster
            </button>
            <button onClick={()=>{ setPasteVal(""); setShowPaste(false); }}
              style={{ background:"transparent", border:"1px solid #e7e0d2", color:"#7d766a", padding:"8px 12px", borderRadius:8, cursor:"pointer", fontSize:12, fontFamily:"'Inter',sans-serif" }}>
              Cancel
            </button>
          </div>
        </div>
      ...
      {busy && (
        <div style={{ marginTop:9, height:5, borderRadius:3, background:"#dfe7d2", overflow:"hidden" }}>
          <div style={{ height:"100%", width:"40%", borderRadius:3, background:"#6dba43", animation:"indeterminate 1.1s ease-in-out infinite" }}/>
        </div>
      )}
    </div>
  );
...
function TenantRoster({ tenants, onTenantClick, asOf, asOfSource }) {
    ...
    <div style={{ background:"#ffffff", border:"1px solid #efe8da", borderRadius:12, padding:"18px 20px", marginBottom:14, boxShadow:"0 1px 2px rgba(56,58,55,0.04)" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, marginBottom:13, flexWrap:"wrap" }}>
        <div style={{ display:"flex", alignItems:"baseline", gap:8, flexWrap:"wrap" }}>
          <span style={{ fontSize:11, letterSpacing:"0.06em", color:"#a69e91", fontWeight:600, textTransform:"uppercase" }}>Tenant Roster — {rows.length===tenants.length?`${tenants.length} tenants`:`${rows.length} of ${tenants.length}`}</span>
          {asOf && <span title={asOfSource==="Rent roll" ? "From an uploaded rent roll" : "From the offering memorandum — upload a current rent roll to refresh"}
            style={{ fontSize:10, fontWeight:600, letterSpacing:"0.02em", color: asOfSource==="Rent roll" ? "#3f7a1f" : "#b08968", background: asOfSource==="Rent roll" ? "#6dba4318" : "#b0896815", border:"1px solid "+(asOfSource==="Rent roll" ? "#8cbf6355" : "#b0896840"), padding:"2px 8px", borderRadius:11 }}>
            AS OF {asOf} · {asOfSource==="Rent roll" ? "RENT ROLL" : "OM"}
          </span>}
        </div>
...
function DetailView({ deal:d, allDeals, editor, onEditor, onToggleVerified, onLookupSale, saleBusy, onGetDemo, demoBusy, onBack, onDelete, onUpdate, onCompare, onQuery, onTenantClick }) {
  ...
  useEffect(() => {
    ...
    if (d.imageMeta && (d.imageMeta.cover || d.imageMeta.sitePlan)) {
      idbLoadImages(d.id).then(res => { if (alive) setImgs(res || {}); }).catch(()=>{ if (alive) setImgs({}); });
    } else {
      // No stored images (e.g. a deal imported without its OM PDF, to save tokens)
      // — still show the cover / site-plan panels so images can be uploaded directly.
      setImgs({});
    }
    return () => { alive = false; };
  }, [d.id]);
  ...
  // ── Direct image uploads (cover photo + site plan) ───────────────────────────
  // For deals that arrived WITHOUT their OM PDF (e.g. bulk-imported to save tokens),
  // there are no captured PDF pages to pick from. These let the user attach a cover
  // photo and site-plan image(s) straight from their device, the same way a normally
  // uploaded deal shows them.
  const _fileToImage = (file) => new Promise((resolve, reject) => {
    const url = URL.createObjectURL(file);
    const im = new Image();
    im.onload = () => resolve({ img: im, revoke: () => URL.revokeObjectURL(url) });
    im.onerror = () => { URL.revokeObjectURL(url); reject(new Error("Couldn't read that image file.")); };
    im.src = url;
  });
  ...
  const handleCoverImageFile = async (e) => {
    ...
    if (e.target) e.target.value = "";
    if (!file) return;
    if (!/^image\//.test(file.type)) { window.alert("Please choose an image file (JPG or PNG)."); return; }
    setUploadingCover(true);
    try {
      const { img, revoke } = await _fileToImage(file);
      const cover = _scaleCanvas(img, 1200, 0.82);
      const thumb = _scaleCanvas(img, 168, 0.5);
      revoke();
      const current = (await idbLoadImages(d.id)) || {};
      const next = { ...current, cover, coverThumb: thumb };
      await idbSaveImages(d.id, next);
      setImgs(next);
      onUpdate(d.id, { imageMeta: { ...(d.imageMeta || {}), cover: true } });
    } catch (err) {
      window.alert(err && err.message ? err.message : "Couldn't read that image.");
    } finally { setUploadingCover(false); }
  };
  const sitePlanImgRef = useRef(null);
  const [uploadingPlan, setUploadingPlan] = useState(false);
  const handleSitePlanImageFiles = async (e) => {
    const files = e.target.files ? Array.from(e.target.files) : [];
    if (e.target) e.target.value = "";
    const picked = files.filter(f => /^image\//.test(f.type));
    if (!picked.length) { if (files.length) window.alert("Please choose image files (JPG or PNG)."); return; }
    setUploadingPlan(true);
    try {
      const current = (await idbLoadImages(d.id)) || {};
      const existing = Array.isArray(current.sitePlan) ? current.sitePlan : [];
      const added = [];
      for (const f of picked) {
        const { img, revoke } = await _fileToImage(f);
        added.push(_scaleCanvas(img, 1500, 0.82));
        revoke();
      }
      const sitePlan = [...existing, ...added];
      const next = { ...current, sitePlan, pagePicks: [], needsSitePlanPick: false };
      await idbSaveImages(d.id, next);
      setImgs(next);
      onUpdate(d.id, { imageMeta: { ...(d.imageMeta || {}), sitePlan: sitePlan.length, needsSitePlanPick: false } });
    } catch (err) {
      window.alert(err && err.message ? err.message : "Couldn't read those images.");
  ...
  const renderSitePlanFixer = (label) => (
    <div style={{ marginTop:12, paddingTop:12, borderTop:"1px solid #f1eadc", display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
      ...
      <span style={{ width:1, height:18, background:"#ece5d7" }}/>
      <span style={{ fontSize:11, color:"#a69e91" }}>or</span>
      <button onClick={()=>sitePlanImgRef.current?.click()} disabled={uploadingPlan}
        style={{ background:"transparent", border:"1px solid #6dba43", color:uploadingPlan?"#a69e91":"#3f6b24", padding:"5px 12px", borderRadius:6, cursor:uploadingPlan?"default":"pointer", fontSize:11, fontWeight:600, fontFamily:"'Inter',sans-serif" }}>
        {uploadingPlan?"Uploading\u2026":"\u2b06 Upload image(s)"}
      </button>
  ...
  const renderCoverFixer = (label) => (
    <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
      ...
      <span style={{ width:1, height:18, background:"#ece5d7" }}/>
      <span style={{ fontSize:11, color:"#a69e91" }}>or</span>
      <button onClick={()=>coverImgRef.current?.click()} disabled={uploadingCover}
        style={{ background:"transparent", border:"1px solid #6dba43", color:uploadingCover?"#a69e91":"#3f6b24", padding:"5px 12px", borderRadius:6, cursor:uploadingCover?"default":"pointer", fontSize:11, fontWeight:600, fontFamily:"'Inter',sans-serif" }}>
        {uploadingCover?"Uploading\u2026":"\u2b06 Upload a photo"}
      </button>
      <input ref={coverImgRef} type="file" accept="image/*" style={{ display:"none" }} onChange={handleCoverImageFile}/>
    </div>
  );
  ...
  return (
    <div style={{ flex:1, overflowY:"auto", padding:"20px 24px" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap", marginBottom:6 }}>
        ...
        {d.autoPassed && (
          <span title={`Auto-passed after 2+ months as a prospect${d.autoPassedAt?` on ${new Date(d.autoPassedAt).toLocaleDateString()}`:""}. Change the status to clear this.`}
      ...
      <p style={{ color:"#6f6a5f", fontSize:12, margin:"0 0 14px 0" }}>{d.address}</p>
      ...
      {/* Edit metrics — kept at the top so it's easy to reach while verifying figures */}
      ...
      <RentRollImport deal={d} onUpdate={onUpdate}/>
      {(d.tenants||[]).length>0 && <TenantRoster tenants={d.tenants} onTenantClick={onTenantClick} asOf={(d.tenantsAsOf||d.omDate)||null} asOfSource={d.tenantsAsOf?"Rent roll":"OM"}/>}
      ...
      {/* Shadow anchors / unowned parcels */}
      {d.shadowAnchors && (
