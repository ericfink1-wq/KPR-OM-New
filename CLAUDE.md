...
- **Movie theaters are judged on sales PER SCREEN, not PSF (Eric, 7/2/26).** A cinema is a big box, so its sales/SF is meaningless; theaters underwrite on sales per screen (and per seat). `theaterMetrics.ts` (`isCinema`, `resolveScreens`, `cinemaSalesRead`) captures the screen count — from the extracted `screens` field (Tenant), else parsed from the name (a trailing number on a cinema, "Regal … 14" → 14) — and derives sales/screen (salesPSF×SF ÷ screens), shown wherever theater sales render (roster, Tenant page) and fed to the grade (roster-analysis snapshot + a CINEMA signal in the prompt; ~$400–700k+/screen is healthy). **SAFEGUARDS:** (a) a real theater rarely exceeds ~30 screens, so a screen count >30 (or <3) is treated as SUSPECT — never silently computes a per-screen figure, shows a "⚠ confirm" prompt, and `audit-screens-implausible` flags it. (b) **Theaters report sales PER SCREEN or PER SEAT, and the extraction once grabbed Regal's "Sales / Screen $165,836" and stored it as salesPSF (a ~1000× units error → an impossible $165,836/SF).** The EXTRACTION_PROMPT salesPSF note now warns never to put a per-screen/per-seat figure in salesPSF (capture `screens` + derive salesPSF = per-screen×screens÷SF); `audit-sales-psf-implausible` (extractionAudit.ts, >$5,000/SF) catches it; and `cinemaSalesRead` refuses to emit a per-screen figure above ~$5M/screen. Real Regal Trussville: 16 screens, ~$165,836/screen (declining — a weak cinema), ~$42/SF, $2.65M total. Extend with per-seat if Eric asks.
...
- **CO-TENANCY "X of N" GUARDRAIL (built 9/1/26 — do NOT weaken).** The worst co-tenancy error is collapsing an "X of N" requirement ("2 of the following must be open and operating: i) Hobby Lobby ii) Belk iii) Ross") into a per-anchor OR, which claims ONE anchor going dark trips it. On Town N' Country (BCA/Easley SC) that turned Belk's Tier-1 exposure into $1,015,131 across 6 tenants when the truth was $68,875 across 1 — a ~15x overstatement of the headline risk number. Root cause was NOT the prompt: `coerceTriggerNode` in `leaseRiskExtract.ts` silently DROPPED `anchors`/`openRequired`/`totalNamed`, so even a correct `anchor_count_below` degraded to a bare per-anchor trigger. Four layers now defend it, all keyed off the clause's OWN `verbatimQuote` (a clause contradicting itself is a deterministic contradiction, never a guess): (1) `coTenancyStructure.ts` (`parseXofN` / `checkCoTenancyStructure` / `repairCoTenancyTrigger`) — the canonical detector, **mirrored verbatim into `om-database/src/lib/coTenancyStructure.ts` with a test that fails if the two drift**; (2) `normCoTenancy` SELF-HEALS at extraction, but **only when the quote's named list matches the anchors the model already chose** — no store name is ever invented or dropped, only the count the quote states verbatim is applied; a disagreeing list is left alone and flagged `check-cotenancy-structure` (high); (3) `audit-cotenancy-xofn` in `extractionAudit.ts` sweeps deals imported BEFORE the fix and self-heals once corrected, and the human-triggered `/deals/autofix` applies the repair library-wide (`coTenancyFixed` count in the "Clean & re-audit all" summary); (4) a LIVE, token-free amber banner on `LeaseRiskPanel` — the exposure figures render straight off the trigger tree, so the warning has to appear where the number is shown, not only after a re-audit. A standalone occupancy prong ("center must stay >65% occupied", "LL maintains 75% of non-anchor floor area") is ALWAYS preserved as a separate OR branch — never folded into the count. A compound slot ("iii) EITHER Ross or Ulta") is expanded into an explicit OR of AND-branches so every named anchor is still evaluated; never invent a combined "Ross or Ulta" anchor.
  - **TEST GUARDRAILS AGAINST THE RAW EXTRACTED TEXT, NEVER A TIDIED FIXTURE (9/3/26 — the lesson that nearly cost the whole fix).** The first cut of this guardrail passed 17 tests and would have repaired NOTHING on the actual Town N' Country OM. Reason: a rent-roll table extracts **COLUMN-INTERLEAVED**, so the real text reads `"i) Hobby Lobby 3% Non-Cumulative CAM Cap Option 3 $12.00 ii) Belk on Controllables Option 4 $12.50 iii) Either Ross or Ulta (Excludes utilities and snow)"` — recovery-method and option-rent columns glued into every enumerated slot. Reconciling by SET EQUALITY of parsed names against the trigger's anchors failed on all five X-of-N clauses (it flagged, correctly, but fixed nothing). What survives interleaving is (a) the COUNT, contiguous right after "X of the following", and (b) each anchor NAME inside its own slot. So reconciliation is by **token-boundary CONTAINMENT** (`"Ross"` must not match `"Wake Forest Crossing"`), mapping each trigger anchor to the slot it lands in and requiring a clean 1:1 placement. Two anchors in ONE slot = a compound slot: expanded to an OR of AND-branches over the failing slot combinations, but ONLY when the tokens strictly BETWEEN the two names are a plain "or" (an "and" means the opposite — the slot fails when EITHER goes dark — so it flags instead). `coTenancyRealOM.test.ts` (om-database) runs the verbatim interleaved quotes through the live exposure engine and pins Belk Tier-1 at $68,875 (the un-repaired path still reproduces the $1,015,131 bug, so the test proves the fix rather than the fixture). **Any future extraction guardrail gets the same treatment: pull the real PDF text and test on that.**
