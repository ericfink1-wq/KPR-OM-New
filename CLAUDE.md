...
7. **RENT-ROLL RECONCILIATION IS A HARD GATE — the rent roll wins.** Before finalizing any abstract, compare its **premises SF, commencement date, current rent, expiry and option count** against the rent roll line for that tenant. If ANY of these disagree, STOP — you almost certainly have the wrong or an incomplete document set. The rent roll is management's current record of the *operative* lease; do NOT rationalize a mismatch away (e.g. "downsizing doc not in folder"). A GLA mismatch (roll says 16,724; your lease says 30,000) means you are abstracting the WRONG LEASE. (This caused the OfficeMax/Office Depot error: the operative doc was a 2015 Amended & Restated Lease for 16,724 SF, not the 1996 lease for 30,000 SF.)
8. **Find the OPERATIVE lease across the WHOLE Drive — don't anchor to the first folder.** A keyword search finds *a* folder, not necessarily the current one. Search the tenant across ALL of Drive (try both brand names — "OfficeMax" AND "Office Depot", store-number variants, "Amended and Restated"). Folder/file names are signals: **"(Original)", "Old", "Superseded", "Prior" mean a NEWER lease exists elsewhere — go find it.** An **"Amended and Restated Lease" supersedes the original entirely** (relocations, downsizings and re-tenanting often come as a new/A&R lease, not as an amendment to the old one). Identify the single operative lease (most recent Lease / A&R Lease) before reading amendments.
9. **Anchor identity to the rent roll + most-recent doc:** tenant legal name, store number, SF, suite, commencement and current rent should all match between your operative lease (as amended) and the rent roll. If the store number or SF differs from what you're reading, you have the wrong document.
