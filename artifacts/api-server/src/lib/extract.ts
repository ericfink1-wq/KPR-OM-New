...
7. leaseType is the REIMBURSEMENT / LEASE STRUCTURE (NNN, Gross, Modified Gross, NN, Base Year) — NOT a renewal-option type. NEVER put option codes like "AUT" (automatic) or "REN" (renewal) in leaseType; that detail belongs in renewalOptions, or omit it. Leave leaseType null if the structure isn't stated. Always capture each tenant's suite/unit id when present (the value in the leftmost "Suite"/"Suite/Space"/"Unit" column, separate from the tenant name) — it's the key used to match tenants across documents. CAPTURE IT EVEN WHEN PURELY NUMERIC: "38", "1", a zero-padded code like "00002"/"00011", or a range "23-24"/"19N20". A leading number before the tenant name on a rent-roll row IS the suite, NOT a row number — put it in the suite field (you may strip leading zeros), and never fold it into the tenant name. MULTI-PHASE / MULTI-BUILDING CENTERS: when a property has more than one phase or building and the suite numbering RESTARTS per phase/building (so the same suite number — Suite 1, 2, 3 — appears in more than one phase), QUALIFY each suite with its phase/building so every suite is UNIQUE across the roster: e.g. "I-1" / "II-1", "A-101" / "B-101", or "Ph2-12". This is legitimate — two different tenants really do share the bare number across phases — so make the suite itself distinguish them (the phase/building goes in the suite field, never in the brand name). Only do this when the source shows repeating suite numbers across phases/buildings.
...
12. SALE COMPARABLES ("comparableSales") ARE PROPERTIES THAT ACTUALLY TRADED — NOT THE COMPETITION. A row belongs in comparableSales ONLY if the OM gives it a SALE PRICE and/or a SALE DATE. Look for the page headed "Sale Comparables", "Comparable Sales", "Recent Transactions", "Investment Sales" or "Sales Comps" — a table with price, date, cap rate, $/SF. NEVER put the OM's COMPETITION or trade-area content here: a "Competitive Set", "Area Retail", "Nearby Centers", "Competition" section or trade-area map lists neighbouring centres with SF, occupancy and anchor names but NO price and NO sale date. Those are the competition, not sale comparables, and putting them here pollutes the comp database with rows that can never support a benchmark. If a candidate row has NO salePrice AND NO saleDate, OMIT IT ENTIRELY rather than emitting a name-and-address shell. An EMPTY comparableSales array is the correct answer when the OM has no sales-comp page — most OMs don't.
...
OCCUPANCY COST IS SECTOR-RELATIVE — never judge a tenant's occupancyCost % against one universal cutoff. What is "healthy" vs "stressed" tracks the sector's gross margin: a thin-margin, high-volume tenant is stressed at a level that is perfectly healthy for a high-margin one. Approximate healthy / stressed occupancy-cost bands by sector (occ cost = rent+recoveries+% rent ÷ sales): Grocery & necessity / warehouse club / supercenter ~healthy ≤3%, stressed >5%; Pharmacy/drug ≤5% / >8%; Off-price & discount ≤6% / >9%; Home improvement (big box) ≤6% / >10%; Automotive ≤8% / >12%; Restaurants/food ≤8% / >12%; Specialty (pet, books, electronics, cosmetics) ≤11% / >16%; Fitness / medical / personal services / entertainment ≤12% / >18%; Apparel & soft goods ≤14% / >18% (healthy 10–15%). So e.g. a grocer at 4% is ELEVATED (not low), and an apparel tenant at 12% is HEALTHY (not high). When you call an occupancy cost "low/healthy" or "high/stressed" — in the below-/above-market logic, strengths, risks, or grade — judge it against the tenant's OWN sector band above, not a flat number. Banks/credit unions report deposit volume, not retail sales, so their "occupancy cost" is not comparable — don't flag it. Cinemas are judged per screen, not PSF.
