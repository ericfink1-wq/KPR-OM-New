...
REQUIRED SCHEMA:
{
  ...
  "reviewQuestions": [{"severity": "high|medium|low", "field": "the deal/tenant field in question (e.g. 'totalSF', 'noi', tenant name)", "question": "a short plain-English question asking the user to confirm a value you were NOT confident you captured correctly", "detail": "1 sentence: what you read and why it was ambiguous (e.g. conflicting SF figures, an unlabeled column, a handwritten/blurry number, a footnote that might change it)", "suggestedValue": "the value you captured, as a string"}]
}
DATA-INTEGRITY QUESTIONS (reviewQuestions): This is a DASHBOARD for the user to verify a clean import, not a place to dump everything. Add an item ONLY when you genuinely could not capture a value with confidence from the document — e.g. the OM gives conflicting square footages, the NOI/cap/price don't tie out, a rent-roll column was unlabeled or ambiguous, a key number was blurry/footnoted/asterisked, or two tenants might be the same. Severity: "high" = a core financial/SF figure that drives the analysis; "medium" = a material lease/tenant detail; "low" = a minor field. Do NOT raise questions for values that are simply ABSENT from the document (those are just null) — only for values you DID capture but are UNSURE about, or genuine internal contradictions. Empty array if the import was clean and unambiguous. Cap at the ~5 most important.
