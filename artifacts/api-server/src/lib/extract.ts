...
LANGUAGE (notes/rationale narration): RENTS DO NOT "TRADE" — properties trade, rents do not; say a rent "is X% below/above" market, never "trades below/above." Reserve "portfolio"/"KPR portfolio" for assets the owner holds; call the broader analyzed dataset "the database." KPR underwrites a 5–7 year hold, so frame any mark-to-market / value-add upside as what's capturable within 5–7 years.
...
- KPR underwrites a 5–7 YEAR HOLD. "Capturable" means realizable within that 5–7 year window — rent locked in by term or options beyond ~7 years is generally NOT upside on a normal hold.
- Below-market rent is only real MARK-TO-MARKET UPSIDE when the landlord can actually capture it during the 5–7 year hold. The strongest case (priority "high") is a tenant that is BELOW market AND has LITTLE LEASE TERM REMAINING (low remainingTermYears / near-term leaseExpiry) AND few or no remaining fixed-rate renewal options (renewalOptions) AND demonstrates it can pay more — STRONG sales (salesPSF) with a LOW occupancyCost %. That tenant can be marked to market at rollover.
- RENEWAL OPTIONS GATE: remaining renewal options put control in the TENANT'S hands — they will exercise only if the option rent is at/below market. So if a below-market tenant holds remaining options at fixed or unspecified (assume non-market) rents, you generally CANNOT mark it to market during the hold; this is NOT upside. Two 5-year options ≈ ~10 years of tenant-controlled occupancy. Below-market rent is capturable upside despite options ONLY if the OM states the options reset to market/FMV. Near-term expiry does NOT override unexercised options.
- DISCOUNT the upside when the tenant has many years of term left, OR has remaining options at fixed/below-market rents (you would never reach market during the hold) — in that case below-market rent is locked in, not upside. Mention it at most as minor/"low".
