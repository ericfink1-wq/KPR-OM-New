  ...
  if (asOfMs != null && occupied.length >= 3) {
    ...
    if (expired.length > 0) {
      ...
      out.push({
        ...
        field: "Lease expiry",
        question: `${expired.length === 1 ? "One lease" : `${expired.length} leases`} (${sf(expiredSF)}, ${sharePct.toFixed(1)}% of GLA) ${expired.length === 1 ? "shows" : "show"} an expiry BEFORE the roster's own as-of date of ${asOfLabel}${anchors.length > 0 ? `, including ${anchors.length === 1 ? "the anchor" : `${anchors.length} anchors`}` : ""}. Has an option been exercised or an amendment signed that isn't reflected here — or ${expired.length === 1 ? "is that tenant" : "are those tenants"} holding over?`,
