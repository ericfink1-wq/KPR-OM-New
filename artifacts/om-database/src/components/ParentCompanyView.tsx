...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ background:"#fff", border:"1px solid #efe8da", borderRadius:10, padding:"12px 16px", flex:"1 1 120px", minWidth:110, boxShadow:"0 1px 2px rgba(56,58,55,0.04)" }}>
      <div style={{ fontSize:10, color:"#a69e91", fontWeight:600, letterSpacing:"0.06em", textTransform:"uppercase", marginBottom:5 }}>{label}</div>
      <div style={{ fontSize:22, fontWeight:700, color:"#26281f", fontFamily:"'Fraunces',Georgia,serif" }}>{value}</div>
    </div>
  );
}
...
export default function ParentCompanyView({ parentName, deals, onBack, onTenantClick, onOpenDeal }: Props) {
  ...
  const subtitle = `${uniqueBrands} brand${uniqueBrands !== 1 ? "s" : ""} · ${allRows.length} location${allRows.length !== 1 ? "s" : ""}`;
  ...
  return (
    <div style={{ flex:1, overflowY:"auto", padding:"20px 24px" }}>
      ...
      <div style={{ fontSize:13, color:"#9a917f", marginBottom:18 }}>{subtitle}</div>

      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16 }}>
        <div style={{ display:"flex", gap:11, flexWrap:"wrap", flex:1 }}>
        ...
        <div style={{ display:"flex", border:"1px solid #e7e0d2", borderRadius:7, overflow:"hidden", fontFamily:"'Inter',sans-serif", fontSize:12, flexShrink:0, marginLeft:12 }}>
          {(["all","owned"] as const).map(s => (
            <button key={s} onClick={() => setScope(s)}
              style={{ padding:"4px 13px", background: scope===s ? "#383a37" : "#faf7f0", color: scope===s ? "#f6f2ea" : "#7d766a", border:"none", cursor:"pointer", fontWeight: scope===s ? 600 : 400, fontFamily:"inherit", fontSize:"inherit" }}>
              {s === "all" ? "All" : "Owned"}
            </button>
          ))}
        </div>
